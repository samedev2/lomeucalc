---
name: lomeucalc
description: Ativa raciocínio hiper objetivo para cálculos, análises e decisões — matemática/financeiro, viabilidade, comparação de opções com pontuação (scoring), estimativas/previsões com confiança declarada (70–80%), sensibilidade/impacto de uma variação (derivativo), e problemas com múltiplas variáveis (análise fracionada). Use sempre que o usuário pedir para calcular, estimar, prever, comparar opções, decidir entre alternativas, avaliar viabilidade, medir impacto de uma mudança, ou pontuar/rankear algo — mesmo sem a palavra "calcular". Vale também para análises de sistemas, arquiteturas ou softwares complexos que peçam conclusão objetiva, não exploração aberta. Respostas curtas e diretas com dados já disponíveis, sem pesquisa desnecessária; ideias criativas só se pedidas explicitamente.
---
